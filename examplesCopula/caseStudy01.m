...
% The analysis used the Generalized Pareto Distribution (GPD) for univariate
% margins and a time-varying Gumbel copula to model dependence. Univariate 
% non-stationary margins were treated based on the method of Mentaschi, et al., 2016
% Univariate peaks were selected with a minimum separation of 30 days, while joint
% extremes were defined as events occurring within a maximum interval of 45
% days, ensuring that high river discharge followed extreme wave events.
% Non-stationarity was assessed within a 40-year moving window, with
% thresholds set at the 95th percentile for river discharge and the 99th
...
% [2] Tilloy, A., Paprotny, D., Grimaldi, S., Gomes, G., Bianchi, A., Lange,
%     S., Beck, H., & Feyen, L. (2024). HERA: a high-resolution pan-European
%     hydrological reanalysis (1950-2020). Earth Syst. Sci. Data Discuss.,
%     2024, 1–38. https://doi.org/10.5194/essd-2024-41
...
[copulaAnalysisNonStat] = tsCopulaExtremes(timeSeriesRiver(:,1), ...
...
[copulaAnalysisNonStat] = tsCopulaCompoundGPDMontecarlo(copulaAnalysisNonStat,...
    'nResample',1000,'timeIndex','last');
[gofStatistics] = tsCopulaGOFNonStat(copulaAnalysisNonStat);


axxArray = tsCopulaPlotBivariate(copulaAnalysisNonStat,gofStatistics, ...
    'ylbl', {'River discharge (m^3s^{-1})','SWH (m)'});
    
[copulaAnalysisForCount] = tsCopulaExtremes(timeSeriesRiver(:,1), ...
    [timeSeriesRiver(:,2),timeSeriesSWH(:,2)], ...
    'minPeakDistanceInDaysMonovarSampling',minDeltaUnivarSampli, ...
    'maxPeakDistanceInDaysMultivarSampling',maxDeltaMultivarSampli, ...
    'copulaFamily',copulaFamily,...
    'transfType','trendlinear','timeWindow',timeWindowJointDist,...
    'ciPercentile',ciPercentile,'potPercentiles',potPercentiles,...
    'peakType','anyExceedThreshold', ...
    'marginalDistributions','gpd','samplingOrder',[1,2]);
[~,hFig1]=tsPlotBivarReturnPeriod(copulaAnalysisNonStat,axxArray,'xlbl','River discharge (m^3s^{-1})','ylbl','SWH (m)','copulaAnalysisForCount',copulaAnalysisForCount,'timeWindowNonStat',timeWindowJointDist/365.25);
