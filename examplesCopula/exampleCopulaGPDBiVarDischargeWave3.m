...
clearvars -except timeAndSeries1 timeAndSeries2

plotyy(datetime(datevec(timeAndSeries1(:,1))),timeAndSeries1(:,end),datetime(datevec(timeAndSeries2(:,1))),timeAndSeries2(:,end))
thresholdPercentiles=[95,99]; %threshold levels in each series for sampling of data
...
if iscell(copulaAnalysis.jointExtremeMonovariateProb)
    figHnd = tsCopulaTimeVaryingPlot(resampleLevel, copulaAnalysis, 'xlbl', 'Discharge m^3/s', 'ylbl', 'SSH (m)','numberofverticalpanels',3,'numberofhorizontalpanels',3);
    ...
    figHnd = tsCopulaTimeVaryingPlot(resampleLevel, copulaAnalysis, 'xlbl', 'Discharge m^3/s', 'ylbl', 'SSH (m)','numberofverticalpanels',1,'numberofhorizontalpanels',1);
